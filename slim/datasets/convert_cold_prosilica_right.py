...
def _get_filenames_and_classes(dataset_dir):
    ...
    """
    cpr_root = os.path.join(dataset_dir, '') #'/prosilica_right/')
    print('-' + dataset_dir  + ':' + cpr_root + '-')
    directories = []
    class_names = []
    for filename in os.listdir(cpr_root):
        path = os.path.join(cpr_root, filename)
...
def _clean_up_temporary_files(dataset_dir):
    ...
    for class_name in _CLASSES:
        tmp_dir = os.path.join(dataset_dir, class_name)
...
def run(dataset_dir):
    ...
    print('\nFinished converting the Cold::Prosilica Right dataset!')
